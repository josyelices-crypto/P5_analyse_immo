...
SELECT 
    ...
    P.Population_Totale
FROM COMMUNE C
JOIN POPULATION_FINALE P ON C.Code_Commune = P.Code_Commune
...
SELECT 
    ...
    ROUND((COUNT(T.ID_Bien) * 1000.0) / C.Population_Totale, 2) AS transactions_pour_1000_hab
FROM COMMUNE C
JOIN BIEN T ON C.Code_Commune = T.Code_Commune
WHERE C.Population_Totale > 10000
GROUP BY C.Nom_Commune, C.Code_Commune, C.Population_Totale,
ORDER BYtransactions_pour_1000_hab DESC
LIMIT 20;
