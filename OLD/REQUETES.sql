...
SELECT 
    ...
    P.opulation
FROM COMMUNE C
JOIN POPULATION_COM P ON C.Code_Commune = P.Code_Commune
...
SELECT 
    ...
    C.code_propre,
    C.population,
    COUNT(T.ID_Bien) AS nombre_transactions,
    ROUND((COUNT(T.ID_Bien) * 1000.0) / C.population, 2) AS transactions_pour_1000_hab
FROM COMMUNE C
JOIN BIEN T ON C.code_propre = T.Code_Commune
WHERE C.population > 10000
GROUP BY C.nom_propre, C.code_propre, C.population,
LIMIT 10;
...
SELECT 
    ...
    -- Formule : (Nombre de transactions / Population) * 1000
    -- On utilise 1000.0 pour forcer le calcul en décimal (float)
    ROUND((COUNT(T.id_transaction) * 1000.0) / P.Population, 2) AS transactions_pour_1000_hab
FROM POPULATION_COM P
JOIN TRANSACTIONS_IMMOBILIERES T ON P.Code_Commune = T.code_insee
WHERE P.Population > 10000
GROUP BY P.Nom_Commune, P.Code_Commune, P.Population
